...
object SparkGpuKMeans {	
	...
	def main(args: Array[String]) {
		...
		val pointsNum = lines.count().toInt //0.691s     
		val data = lines.map(parseVector _).cache()
		
		var collectTime : Double= System.nanoTime()
		val pointsArray = data.collect().toArray //3.115s
		...
		val kPointsArray = data.take(K).toArray //cluster points, 0.012s 
		...
		*/
		...
		while (tempDist > convergeDist) {
			...
			val closest = zipRdd.map (p => (p._1, (p._2, 1))) // be careful!! 0.511s
